...
class ParetoLogger(nenv.logger.AbstractLogger):
    """
        This method evaluate the pareto estimation for NegoFormer agent.
    ...
    def on_offer(self, agent: str, offer: Bid, time: float, session: Union[Session, SessionEstimator]) -> LogRow:
        ...
        if self.agent_pos != agent:
            return {"NegoFormerAgent_Pareto": {"Precision": "-", "Recall": "-", "F1": "-", "TP": "-", "FP": "-", "FN": "-", "MSE": "-", "Avg.Diff": self.average_diff, "WalkIndex": "-"}}
        ...
        if agent.pareto_index >= 0:
            ...
            return {"NegoFormerAgent_Pareto": {"Precision": precision, "Recall": recall, "F1": f1, "TP": tp, "FP": fp, "FN": fn, "MSE": mse, "Avg.Diff": self.average_diff, "WalkIndex": agent.pareto_index}}

        return {"NegoFormerAgent_Pareto": {"Precision": "-", "Recall": "-", "F1": "-", "TP": "-", "FP": "-", "FN": "-", "MSE": "-", "Avg.Diff": self.average_diff, "WalkIndex": "-"}}

    def on_session_end(self, final_row: LogRow, session: Union[Session, SessionEstimator]) -> LogRow:
        if self.agent_pos == "":
            return {"NegoFormerAgent_Pareto": {"Precision": "-", "Recall": "-", "F1": "-", "TP": "-", "FP": "-", "FN": "-", "MSE": "-", "Avg.Diff": "-", "WalkIndex": "-"}}
        ...
        for i, row in enumerate(session.session_log.log_rows["NegoFormerAgent_Pareto"]):
        ...
        return {"NegoFormerAgent_Pareto": {"Precision": precision, "Recall": recall, "F1": f1, "TP": tp, "FP": fp, "FN": fn, "MSE": mse, "WalkIndex": indices, "Avg.Diff": self.average_diff}}

    def on_tournament_end(self, tournament_logs: ExcelLog, agent_names: List[str], domain_names: List[str], estimator_names: List[str]):
        total_f1, total_recall, total_precision, total_mse, total_indices = [], [], [], [], []

        for row in tournament_logs.log_rows["NegoFormerAgent_Pareto"]:
            if "Precision" not in row or row["Precision"] == "-":
                continue
