...
class CandidatesLogger(nenv.logger.AbstractLogger):
    """
        This logger analyzes the candidate selection for ParetoWalker Agent
    ...
    def before_session_start(self, session: Union[Session, SessionEstimator]) -> List[str]:
        ...
        if session.agentA.name == "ParetoWalkerAgent":
            ...
            return ["ParetoWalkerAgent_Candidates"]
    ...
    def on_offer(self, agent: str, offer: Bid, time: float, session: Union[Session, SessionEstimator]) -> LogRow:
        ...
        if self.agent_pos != agent:
            return {"ParetoWalkerAgent_Candidates": {"Selected": "-",
        ...
        if len(agent.last_candidate) > 0:
            ...
            return {"ParetoWalkerAgent_Candidates": row}
    ...
    def on_session_end(self, final_row: LogRow, session: Union[Session, SessionEstimator]) -> LogRow:
        if self.agent_pos == '':
            return {"ParetoWalkerAgent_Candidates": {"Pareto_Slope": "-", "Pareto_AgentUtility": "-", "Pareto_OppUtility": "-", "Pareto_EstOppUtility": "-", "Pareto_SelectionRate": "-",
        ...
        for row in session.session_log.log_rows["ParetoWalkerAgent_Candidates"]:
    ...
    def on_tournament_end(self, tournament_logs: ExcelLog, agent_names: List[str], domain_names: List[str], estimator_names: List[str]):
        ...
        for row in tournament_logs.log_rows["ParetoWalkerAgent_Candidates"]:
