...
from tournament_graphs import plt
...
class UtilityDistributionLogger(AbstractLogger):
    ...
    def draw_agent_opponent_utility(self, tournament_logs: ExcelLog, agent_names: list, directory: str,
        ...
        for i, row in enumerate(tournament_logs.log_rows["UtilityDist"]):
            domain_id = tournament_logs.log_rows["TournamentResults"][i]["DomainID"]
        ...
        for agent_name in opponent_mean_utilities:
            ...
            x_err = [[abs(x - np.min(opponent_min_utilities[agent_name]))], [abs(np.max(opponent_max_utilities[agent_name]) - x)]]
            y_err = [[abs(y - np.min(agent_min_utilities[agent_name]))], [abs(np.max(agent_max_utilities[agent_name]) - y)]]
            ...
            plt.errorbar(x, y, xerr=x_err, yerr=y_err, label=agent_name, marker='o', markersize=5.)
        ...
        with open(os.path.join(directory, "utility_distribution.csv"), "w") as f:
            f.write("AgentName;OpponentMeanUtility;AgentMeanUtility;OpponentMinUtility;AgentMinUtility;OpponentMaxUtility;AgentMaxUtility;\n")
    ...
    def get_pareto_nash_kalai(self, domain_no: int) -> (np.ndarray, BidPoint, BidPoint):
