...
class SessionEstimator:
    """
        SessionEstimator simulates the negotiation to generate Estimator logs after the tournament.
        It takes the previous session log to give them to Estimators
    """
    ...
    def __init__(self, agentA: AbstractAgent, agentB: AbstractAgent, path: str, loggers: list):
        """
            Constructor
    ...
    def process_row(self, row: dict, row_index: int, row_tournament: dict) -> dict:
        ...
        if row["Action"] == 'Accept':
            ...
            row_tournament["TournamentResults"]["NashProduct"] = row["NashProduct"]
            row_tournament["TournamentResults"]["KalaiSum"] = row["KalaiSum"]
    ...
    def process_end(self, row: LogRow) -> LogRow:
        ...
        for logger in self.loggers:
            if 'Result' in row["TournamentResults"] and row["TournamentResults"]["Result"] == 'Acceptance':
                ...
                row["TournamentResults"]["NashProduct"] = agent_a_utility * agent_b_utility
                row["TournamentResults"]["KalaiSum"] = agent_a_utility + agent_b_utility
